      ...
      stations.forEach((station) => {
        ...
        marker.addListener("click", () => {
          ...
          station.business_hours.forEach((hour) => {
            ...
            // available_hour と remarks の取得
            const availableHour = station.stamp_hours.available_hour;
            const remarks = station.stamp_hours.remarks;

            // 表示内容を作成
            if (availableHour === "24時間押印可能") {
              // 24時間押印可能の場合
              stampHoursElement.innerHTML = `
                <div>
                  <span class="stamp-availability">${availableHour}</span>
                  <span class="stamp-notice">※${remarks}</span>
                </div>
              `;
            } else {
              // 開館時間に同じ場合
...
fetch("/maps/stations_data")
  .then((response) => response.json())
  .then((stations) => {
    console.log(stations); // レスポンス内容を確認
    stations.forEach((station) => {
      console.log(station.stamp_hours); // stamp_hoursの内容を確認
    });
  })
  .catch((error) => console.error("Error fetching stations data:", error));
